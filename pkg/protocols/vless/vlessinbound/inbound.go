...
const TypeVlessV2 = "vless_v2"

func Register(registry *inbound.Registry) {
	inbound.Register(registry, TypeVlessV2, NewInbound)
}
...
type Inbound struct {
	...
	service   *vlessproto.VlessService
	tlsConfig tls.ServerConfig
	transport adapter.V2RayServerTransport
}

func NewInbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options VLESSInboundOptions) (adapter.Inbound, error) {
	inbound := &Inbound{
	...
	service := vlessproto.NewVlessService(
		logger,
...
func (h *Inbound) newConnectionEx(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) {
	...
	user, loaded := auth.UserFromContext[vlessproto.User](ctx)
	if !loaded {
