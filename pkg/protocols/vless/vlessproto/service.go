...
func NewVlessService(logger logger.Logger, handler Handler, userStore UserStore) *VlessService {
	return &VlessService{
		logger:    logger,
		handler:   handler,
		userStore: userStore,
...
func (s *VlessService) NewConnection(ctx context.Context, conn net.Conn, source M.Socksaddr, onClose N.CloseHandlerFunc) error {
	...
	if !ok {
		return fmt.Errorf("user uuid '%s' not found in the store", uuid.FromBytesOrNil(request.UUID[:]).String())
	}
	ctx = auth.ContextWithUser(ctx, user)
	userFlow := user.Flow
	if request.Flow == vlessvision.Flow && request.Command == vmess.NetworkUDP {
		return E.New(vlessvision.Flow, " flow does not support UDP")
	...
	switch userFlow {
	case vlessvision.Flow:
		conn, err = vlessvision.NewVisionConn(responseConn, conn, request.UUID, s.logger)
		if err != nil {
